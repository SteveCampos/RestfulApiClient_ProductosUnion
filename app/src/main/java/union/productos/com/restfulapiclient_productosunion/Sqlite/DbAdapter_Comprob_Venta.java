...
public class DbAdapter_Comprob_Venta {
    ...
    public long createComprobVentas(ComprobanteVentaEnv comprobante, int id_agente){

        ContentValues initialValues = new ContentValues();
        initialValues.put(CV_id_establec,comprobante.getEstIEstablecimientoId());
        initialValues.put(CV_id_tipo_doc, comprobante.getComIComprobanteTipoId());
        initialValues.put(CV_id_forma_pago,comprobante.getComIFormaPagoId());
        initialValues.put(CV_id_tipo_venta,comprobante.getComITipoVentaId());
        initialValues.put(CV_codigo_erp,comprobante.getCodigo_erp());
        initialValues.put(CV_serie,comprobante.getComVSerie());
        initialValues.put(CV_num_doc,comprobante.getComINumDoc());
        ...
        initialValues.put(CV_fecha_doc,"");
        initialValues.put(CV_hora_doc,"");
        initialValues.put(CV_estado_comp,0);
        initialValues.put(CV_estado_conexion,"");
        initialValues.put(CV_id_agente,id_agente);

        return mDb.insert(SQLITE_TABLE_Comprob_Venta, null, initialValues);
    }

    public void updateComprobantes(ComprobanteVentaEnv comprobante, int id_agente){
        ContentValues initialValues = new ContentValues();
        initialValues.put(CV_id_establec,comprobante.getEstIEstablecimientoId());
        initialValues.put(CV_id_tipo_doc, comprobante.getComIComprobanteTipoId());
        initialValues.put(CV_id_forma_pago,comprobante.getComIFormaPagoId());
        ...
        mDb.update(SQLITE_TABLE_Comprob_Venta, initialValues,
                CV_id_comprob+"=?",new String[]{""+comprobante.getEstIEstablecimientoId()});
    }
    ...
    public void insertSomeComprobVenta() {
        ...
        createComprobVenta( 1, 1, 1, 1, "0001", "1A", 1, 10, 0, 10, "09/01/2015", "08:10:00", 1, 0, 1);
        createComprobVenta( 2, 1, 1, 1, "0002", "2A", 2, 20, 0, 20, "08/01/2015", "08:10:00", 0, 0, 1);
        createComprobVenta( 3, 1, 1, 1, "0003", "3A", 3, 30, 0, 30, "08/01/2015", "08:10:00", 1, 0, 1);
        createComprobVenta( 4, 2, 2, 1, "0004", "4A", 4, 10, 0, 40, "08/01/2015", "08:10:00", 1, 0, 1);
        createComprobVenta( 5, 2, 2, 1, "0005", "5A", 5, 10, 0, 50, "08/01/2015", "08:10:00", 0, 0, 1);
